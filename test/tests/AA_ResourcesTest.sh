...
#########
TEST_NAME="CreateResourceUnauthed"
SHOULD_STATUS="401"
...
RESPONSE=$(curl -s -o "$OUTPUT_FILE" -w "%{http_code}" --location --request POST http://localhost:8080/de/resources \
...
#########
TEST_NAME="CreateResourceFalseToken"
SHOULD_STATUS="401"
...
RESPONSE=$(curl -s -o "$OUTPUT_FILE" -w "%{http_code}" --location --request POST http://localhost:8080/de/resources \
  --header 'Content-Type: application/json' \
  --header "Authorization: Bearer blub" \
  --data @"$INPUT_FILE"
)
