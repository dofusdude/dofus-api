...
RESPONSE=$(curl -s -o /dev/null -w "%{http_code}" --location --request POST http://localhost:8080/de/resources \
  --header 'Content-Type: application/json' \
  --header "Authorization: Bearer $SECRET" \
  --data @"$INPUT_FILE"
)

RESPONSE=$(curl -s -o "$OUTPUT_FILE" -w "%{http_code}" --location --request GET http://localhost:8080/de/resources/1 \
  --header 'Content-Type: application/json' \
  --data @"$INPUT_FILE"
)
